...
const vehicles = [
  { id: 1, type: 'Sedan', year: '2020', make: 'Toyota', price: 25000, location: 'California' },
  { id: 2, type: 'SUV', year: '2019', make: 'Honda', price: 22000, location: 'Texas' },
  { id: 3, type: 'Sedan', year: '2021', make: 'Nissan', price: 28000, location: 'California' },
  { id: 4, type: 'Truck', year: '2020', make: 'Ford', price: 27000, location: 'Nevada' },
  { id: 5, type: 'Sedan', year: '2020', make: 'Toyota', price: 29000, location: 'California' }
];
...
function displayResults(filteredVehicles) {
  ...
  filteredVehicles.forEach(vehicle => {
      ...
      vehicleElement.innerHTML = `
          <h2>${vehicle.type} - ${vehicle.year}</h2>
          <p>Make: ${vehicle.make}</p>
          <p>Price: $${vehicle.price}</p>
          <p>Location: ${vehicle.location}</p>
          <button onclick="toggleMark(${vehicle.id})" class="${isMarked ? 'marked' : ''}">
...
function viewMarkedTrucks() {
  let markedTrucks = JSON.parse(localStorage.getItem('markedTrucks')) || [];
  let markedVehicles = vehicles.filter(vehicle => markedTrucks.includes(vehicle.id));
    ...
    function applyFilters() {
        ...
        const filteredVehicles = vehicles.filter(vehicle => {
            return (
                ...
                (location === '' || vehicle.location.toLowerCase() === location.toLowerCase())
            );
        });
